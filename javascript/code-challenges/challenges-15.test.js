...
const screenForNames = (arr) => arr.filter(str => /^(?:M|D)(?:r|s)s?. {1}[a-zA-Z]+/.test(str));
...
const toTitleCase = (arr) => arr.map(string => string = string[0].toUpperCase() + string.slice(1));
...
let biggerThanLuke = (arr) => arr.reduce((names, character) => character.mass <= 77 ? names : names ? names + ' - ' + character.name : character.name, '');
...
const sortBy = (property, arr) => arr.sort((itemA, itemB) => itemA[property] < itemB[property] ? -1 : 1);
...
const detectTicTacToeWin = (board) => potentialWins.map(coordSet =>
  board[coordSet[0][0]][coordSet[0][1]]
    && board[coordSet[0][0]][coordSet[0][1]] === board[coordSet[1][0]][coordSet[1][1]]
    && board[coordSet[0][0]][coordSet[0][1]] === board[coordSet[2][0]][coordSet[2][1]] ?
    true : false)
  .includes(true);
