...
const raisedToTheThird = (arr) => {
  let result = [];
  arr.forEach(num => result.push(Math.pow(num, 3)));
  return result;
};
...
const evenOdd = (arr) => {
  return arr.map(num => typeof(num) !== 'number'? 'N/A' : num % 2 === 0? 'even' : 'odd');
};
...
const extractStats = (arr) => {
  return arr.map(item => new Object({name: item.stat.name, total: (item.effort + item.baseStat)}));
};
