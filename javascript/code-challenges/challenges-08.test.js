...
const sortByChildren = (charArray) => {
  charArray.sort((a, b) => a.children.length > b.children.length? 1 
    : a.children.length < b.children.length? -1
      : a.house > b.house? 1
        : -1);
  console.log(charArray); //Euron is at [0], so this test is maybe not working?
};
...
const isCapitalized = (str) => {
  let matchPattern = /\b[A-Z][a-zA-z]*/g;
  let match = str.match(matchPattern);
  return match || [];
};
...
const citiesAtoJ = (arr) => {
  let testPattern = /^[A-J]/g; //For some reason, Boston isn't coming through with this pattern.
  let result = arr.filter(city => testPattern.test(city));
  return result;
};
