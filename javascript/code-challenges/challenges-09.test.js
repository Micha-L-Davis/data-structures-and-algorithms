...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = (arr) => arr.reduce((previous, current) => previous >= current? previous : current);
...
const hasChildrenValues = (arr, character) => Object.values(arr.filter((char) => char.name === character)[0]).filter((value) => Array.isArray(value)).length > 0 ? true : false;

//arr.filter((char) => char.name === character)[0].children? true : false; //uncontrived answer
