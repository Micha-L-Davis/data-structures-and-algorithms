...
function transformToLis(obj){
  ...
  for (let property in obj){
    result.push(`<li>${property}: ${obj[property]}</li>`);
  }
  return result;
}
...
const count = (target, input) => input.reduce((count, row) => count += row.reduce((subCount, num) => num === target? subCount + 1 : subCount, 0), 0);
...
const totalSum = (input) => input.reduce((count, row) => count += row.reduce((subCount, num) => subCount + num, 0), 0);
...
const divisibleByFiveTwoToThePower = (input) => input.map(subArr => subArr
  .filter(char => typeof(char) !== 'string')
  .filter(numToDiv => numToDiv % 5 === 0)
  .map(powNum => Math.pow(2, powNum)));
...
let findMaleAndFemale = (data) => data.filter(character => character['gender']
  .includes('male'))
  .reduce((stringOfNames, character) => stringOfNames? stringOfNames + ' and ' + character['name']
    : character['name'], '');
...
let findShortest = (data) => data.reduce((shortest, character) => +character.height > +shortest.height? shortest : character).name;
